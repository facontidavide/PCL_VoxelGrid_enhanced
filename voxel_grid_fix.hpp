#ifndef BOR_FILTERS_IMPL_VOXEL_GRID_H_
#define BOR_FILTERS_IMPL_VOXEL_GRID_H_
...
struct cloud_point_index_idx
{
  ...
  cloud_point_index_idx(int idx_, unsigned int cloud_point_index_)
...
{
  inline int operator()(const cloud_point_index_idx & x, const unsigned offset)
  {
    return x.idx >> offset;
  }
};
...
class VoxelGridFixed : public pcl::VoxelGrid<PointT>
{
...
public:
  ...
  void applyFilter(PointCloud & output)
  {
    ...
    if (!filter_field_name_.empty()) {
      ...
      for (std::vector<int>::const_iterator it = indices_->begin(); it != indices_->end(); ++it) {
        if (!input_->is_dense)
          ...
          if (
            !std::isfinite(input_->points[*it].x) || !std::isfinite(input_->points[*it].y) ||
        ...
        int ijk0 = static_cast<int>(
          std::floor(input_->points[*it].x * inverse_leaf_size_[0]) -
          static_cast<float>(min_b_[0]));
        int ijk1 = static_cast<int>(
          std::floor(input_->points[*it].y * inverse_leaf_size_[1]) -
          static_cast<float>(min_b_[1]));
        int ijk2 = static_cast<int>(
          std::floor(input_->points[*it].z * inverse_leaf_size_[2]) -
    ...
    boost::sort::spreadsort::integer_sort(
      index_vector.begin(), index_vector.end(), cloud_point_index_rightshift(),
